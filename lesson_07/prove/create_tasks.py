...
def write_dict(file_count, dictionary):
    ...
    files_created += 1
    print(f'Saving to tasks: {get_filename(file_count)}')
    with open(f'tasks/{get_filename(file_count)}', 'w') as f:
...
def main():
    ...
    # Create the tasks dir if it does not exist.
    ...
    print(f'{files_created} task files created.')
